...
def compute_distance_id(G, u, v):
    start_config = helper.state_to_numpy(G.nodes[u]['coords'])
    end_config = helper.state_to_numpy(G.nodes[v]['coords'])
        ...
        while len(queue):
            ...
            for v in next_cur:
                ...
                if (v not in nodes) or nodes[v][0] > dis_v:
                    ...
                    node1_pos = helper.state_to_numpy(G.nodes[v]['coords'])
                    node2_pos = helper.state_to_numpy(G.nodes[cur]['coords'])
